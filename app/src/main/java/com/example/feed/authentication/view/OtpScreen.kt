...
fun OtpScreen(
    ...
    email         : String          // passed from Login or Register
) {
    ...
    LaunchedEffect(otpState) {
        when (otpState) {
            is UiState.Success -> {
                // Clear entire back stack — user is fully authenticated
    ...
    OtpContent(
        ...
        onValueChange   = { index, value ->
            if (value.length <= 1 && value.all { it.isDigit() }) {
                ...
                if (value.isNotEmpty() && index < 5) focusRequesters[index + 1].requestFocus()
                if (errorMsg.isNotEmpty()) errorMsg = ""
            }
        },
        onVerifyClick   = {
            val code = otpValues.joinToString("")
            if (code.length == 6) viewModel.verifyOtp(email, code)
            else errorMsg = "Please enter all 6 digits"
        },
        onResendClick   = {
            resendTimer = 30
            errorMsg    = ""
...
fun OtpContent(
    ...
    Box(
        ...
        Column(
            ...
            Text("Verify your email", fontSize = 26.sp, fontWeight = FontWeight.Bold, color = Color(0xFF1A1A1A))
                ...
                otpValues.forEachIndexed { index, value ->
                    OutlinedTextField(
                        ...
                        colors = OutlinedTextFieldDefaults.colors(
                            ...
                            unfocusedBorderColor = if (value.isNotEmpty()) Primary.copy(alpha = 0.5f) else Color(0xFFDDDDDD),
                            cursorColor          = Primary,
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                if (resendTimer > 0) {
                    Text("Resend in ${resendTimer}s", color = Color(0xFFAAAAAA), fontSize = 14.sp)
                } else {
                    Text("Resend OTP", color = Primary, fontSize = 14.sp, fontWeight = FontWeight.SemiBold,
